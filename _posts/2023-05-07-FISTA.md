...
$$\min_x f(x) = g(x) + h(x)$$
where $g$ is differentiable, $\nabla g$ is L-Lipschitz and $h$ is not necessarily differentiable, proximal gradient method apply gradient descent on $g$. Its update goes in the following fashion:
