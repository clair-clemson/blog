...
1. Initialize $\fm^0 \in \mathcal{C}_F$ and $\vg_j^0 \in \operatorname{relint}(\Delta_k)$ for $j = 1,\ldots,n$.
...
Since $\mathbf{S}=\mathbf{M}^{\top}\mathbf{M}$ is symmetric, $\mathbf{A}$ is symmetric, and therefore $\mathbf{B}$ is also symmetric.
...
The diagonal terms are removed because

$$
A_{ii}=0.
$$

For a fixed column $\mathbf{m}_k$, we compute the derivative. Since
...
with the convention that the derivative at $z=0$ is taken as $0$, we have
