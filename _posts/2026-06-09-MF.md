...
**Remark.** $\overline{L}$ can be used in setting stepsize in each iteration for updating $\fm,\gm$, which is an **oblivious** version, in that it doesn't depend on any quantity arising from the optimization. One can also use **adaptive** rule that recomputes the relevant Lipschitz constants using the updated parameters. However, since our algorithm is essentially an alternating minimization, it is not immediately clear which rule leads to better practical or theoretical performance.
...
Since $\vg_j\in\Delta_k$ implies $\|\vg_j\|_2\le 1$, we have
